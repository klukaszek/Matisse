...
class UNet(eqx.Module):
    ...
    def __init__(self, dim_latent: int = 8, *, key: jax.random.PRNGKey):
        """Initialize UNet.
        ...
        self.inc = DoubleConv(dim_latent, 16, key=keys[0])
        self.down1 = Down(16, 32, key=keys[1])
        self.down2 = Down(32, 64, key=keys[2])
        self.down3 = Down(64, 128, key=keys[3])
        self.up1 = Up(128, 64, False, key=keys[4])
        self.up2 = Up(64, 32, False, key=keys[5])
        self.up3 = Up(32, 16, False, key=keys[6])
        self.outc = OutConv(16, dim_latent, key=keys[7])
...
class DefaultDemosaicing(eqx.Module):
    ...
    def __init__(self, latent_dim: int = 8, *, key: jax.random.PRNGKey):
        ...
        self.demosaicing = UNet(dim_latent=latent_dim, key=key)

    def demosaic(self, C_encoded_pa: Float[Array, "batch latent_dim height width"]) -> Float[Array, "batch latent_dim height width"]:
        """Apply demosaicing.
